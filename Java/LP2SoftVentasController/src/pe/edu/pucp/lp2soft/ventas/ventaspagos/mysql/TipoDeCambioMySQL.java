...
public class TipoDeCambioMySQL implements TipoDeCambioDAO {
    ...
    public int insertarTipoDeCambio(TipoDeCambio tipoDeCambio) {
        ...
        try{
            ...
            cs = con.prepareCall("{call INSERTAR_TIPO_DE_CAMBIO(?,?,?,?,?)}");
            cs.registerOutParameter("_id_tipo_de_cambio", java.sql.Types.INTEGER);
            cs.setInt("_fid_moneda", tipoDeCambio.getFid_Moneda());
            cs.setDate("_fecha", new java.sql.Date(tipoDeCambio.getFecha().getTime()));
            cs.setDouble("_cambio", tipoDeCambio.getCambio());
            cs.setBoolean("_activo", tipoDeCambio.getActivo());
            
            resultado = cs.executeUpdate();
            tipoDeCambio.setIdTipoDeCambio(cs.getInt("_id_tipo_de_cambio"));
        }catch(Exception ex){
            System.out.println(ex.getMessage());
        }finally{
            try{
    ...
    public int modificarTipoDeCambio(TipoDeCambio tipoDeCambio) {
        ...
        try{
            con = DBManager.getInstance().getConnection();
            cs = con.prepareCall("{call MODIFICAR_TIPO_DE_CAMBIO(?,?,?,?,?)}");
            cs.setInt("_id_tipo_de_cambio", tipoDeCambio.getIdTipoDeCambio());
            cs.setInt("_fid_moneda", tipoDeCambio.getFid_Moneda());
            cs.setDate("_fecha", new java.sql.Date(tipoDeCambio.getFecha().getTime()));
            cs.setDouble("_cambio", tipoDeCambio.getCambio());
            cs.setBoolean("_activo", tipoDeCambio.getActivo());
    ...
    public ArrayList<TipoDeCambio> listarTipoDeCambioXNombre(String nombre) {
        ...
        try{
            con = DBManager.getInstance().getConnection();
            cs = con.prepareCall("{call LISTAR_TIPOS_DE_CAMBIO_X_NOMBRE(?)}");
            cs.setString(1, nombre);
            rs = cs.executeQuery();
            while(rs.next()){
                TipoDeCambio tipoDeCambio = new TipoDeCambio();
                tipoDeCambio.setIdTipoDeCambio(rs.getInt("id_tipo_de_cambio"));
                tipoDeCambio.setFid_Moneda(rs.getInt("fid_moneda"));
                tipoDeCambio.setFecha(rs.getDate("fecha"));
                tipoDeCambio.setCambio(rs.getDouble("cambio"));
                tiposDeCambio.add(tipoDeCambio);
            }
